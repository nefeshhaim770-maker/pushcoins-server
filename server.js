...
const bankDetailsSchema = new mongoose.Schema({
    ...
    approvedDate: Date,
    isSetup: { type: Boolean, default: false }, // If true, mandate exists in Kesher
...
async function chargeCreditCard(user, amount, note, creditDetails = null) {
    ...
    // Consistent Identity Logic: TZ if exists, else Phone
    ...
    console.log(`🚀 Sending CC Charge for ${user.name} (ID: ${uniqueId})`);
...
async function createBankObligation(user, amount, note, isRecurring = false) {
    ...
    // If we have an active status AND isSetup flag, we assume mandate is valid at the bank/Kesher
    // and we don't need to re-send SendBankObligation every time.
    if (user.bankDetails.isSetup && user.bankDetails.status === 'active') {
        console.log(`🏦 Bank mandate exists for ${user.name}. Skipping API call.`);
        return {
            success: true,
            data: { message: "Existing mandate used (Local Record)" },
            paymentMethod: 'bank'
        };
    }
    
    // FIXED: Convert Amount to correct unit.
    // If 1 = 10 Agorot, then we need to multiply by 100? No, usually 100 agorot = 1 shekel.
    // If you send '1' and get '0.10', it means they divide by 10. Strange. 
    // Standard ILS is 100 Agorot. So 1 Shekel should be 100.
    // If you send 1 and want 1 Shekel, we send 1.
    // Assuming standard: Send 1.00 for 1 Shekel if they take float. If they take Int Agorot, send 100.
    // The CURL used "Total: 500". If that meant 500 Shekels, then send plain number.
    const totalAmount = parseFloat(amount); // Trying straight Shekels based on CURL "500"

    const bankPayload = {
        ClientApiIdentity: uniqueId, // Ensure we link to same client ID
        Signature: null,
        ...
        NumPayment: isRecurring ? 9999 : 1, // 1 for one-time charge, 9999 for standing order setup
        
        // FIXED: Using UserId as per error log
        UserId: user.bankDetails.ownerID || user.tz || "000000000" 
    };

    console.log(`🏦 Sending Bank Setup for ${user.name} (ID: ${uniqueId})`);
    ...
    // Check success
    // If response is just the ID (number) or success object
    const isSuccess = !res.data.error && (res.data.status !== 'error');
    
    if(isSuccess) {
        // Mark as setup so we don't send again
...
async function performCharge(user, amount, note, forceCC = false, creditDetails = null) {
    if (forceCC || user.preferredPaymentMethod === 'cc' || creditDetails) {
        return await chargeCreditCard(user, amount, note, creditDetails);
         ...
         // Logic: If user is making a manual donation ("One Time"), we don't necessarily want to set up a new 9999 mandate.
         // However, if we reuse existing mandate, we just record it.
         // If mandate doesn't exist, we must create it.
         // For one-time donation via bank, we send NumPayment=1.
            ...
            if (u.recurringImmediate === true || u.billingPreference === 0) {
                if(canCharge) {
                    try {
                        let r;
                        if(isBank) {
                            // If bank recurring is active, we assume we process it via file or existing mandate
                            // createBankObligation checks if isSetup=true and skips re-sending if so
                            r = await createBankObligation(u, amountToCharge, "הוראת קבע יומית", true);
                        } else {
        ...
        if (action === 'approve') {
            try {
                ...
                if (kesherRes.success) {
                    ...
                    const msg = kesherRes.data?.error || kesherRes.data?.faultstring || "שגיאה לא ידועה";
                    throw new Error(msg);
                }
            } catch(err) {
