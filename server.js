...
try {
    ...
    } catch (err) { console.log("⚠️ Warning: No Firebase Key"); }
}
...
// ✅ התיקון הגדול לשגיאות תרומה
app.post('/donate', async (req, res) => {
    const { userId, amount, useToken, note, forceImmediate, ccDetails } = req.body;
    let u = await User.findById(userId);
    
    // אם המשתמש הוא לחיוב מיידי (0) או שביקש מיידי
    const shouldChargeNow = forceImmediate === true || u.billingPreference === 0;
    ...
    if (shouldChargeNow) {
        try {
            ...
            if (r.success) {
                ...
                // בדיקת סוג השגיאה והחזרת הודעה מתאימה
                let rawError = r.data.Description || "סירוב לא ידוע";
                let finalError = rawError;

                // בדיקה אם המילה "Duplicate" או "כפולה" מופיעה בשגיאה
                if (/Duplicate|double|כפולה/i.test(rawError)) {
                    finalError = "הסכום צריך להיות שונה שלא יחשב כתרומה כפולה";
                }

                res.json({ success: false, error: finalError });
            }
        } catch(e) { 
            res.json({ success: false, error: "תקלת תקשורת: " + e.message }); 
        }
    } else {
