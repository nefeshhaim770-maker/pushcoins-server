...
// ============================================================
// ⚙️ הגדרות המייל - התיקון הסופי ל-ETIMEDOUT
// ============================================================
const transporter = nodemailer.createTransport({
    host: 'smtp.gmail.com',
    port: 587,              // ✅ פורט 587 יציב יותר בשרתי ענן
    secure: false,          // ✅ חובה להיות false בפורט 587
    auth: {
        user: process.env.EMAIL_USER, // לוקח את המייל מ-Render
        pass: process.env.EMAIL_PASS  // לוקח את הסיסמה החדשה מ-Render
    },
    tls: {
        rejectUnauthorized: false
    },
    family: 4 // ⚠️ הפקודה הקריטית: מונעת ניתוקים ב-Render (מכריח IPv4)
});
...
transporter.verify((error, success) => {
    if (error) {
        ...
        console.log("✅ השרת מחובר לג'ימייל (פורט 587, IPv4) ומוכן לשליחה!");
    }
});
// ============================================================
    ...
    try {
        ...
        if (cleanEmail) {
            ...
            try {
                ...
                console.error("❌ שגיאה בשליחת המייל (בתוך הבקשה):", mailError);
                // במקרה של שגיאה נחזיר הודעה ברורה
                res.status(500).json({ success: false, error: "תקלה בשליחת המייל. נסה שוב." });
            }
        } else {
            // במקרה של SMS (שלא פעיל כרגע)
