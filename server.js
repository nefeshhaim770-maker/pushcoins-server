...
// --- Bank Obligation (EXACTLY AS PER CURL) ---
async function createBankObligation(user, amount, note) {
    ...
    const bankPayload = {
        ClientApiIdentity: user.bankDetails.ownerID || null, // Using ownerID if exists, else null (CURL said null but ID is usually needed)
        Signature: null,
        Account: parseInt(user.bankDetails.accountId), // Must be Integer
        Branch: parseInt(user.bankDetails.branchId),   // Must be Integer
        Bank: parseInt(user.bankDetails.bankId),       // Must be Integer
        Address: "Israel", // Placeholder
        City: null,
        Total: parseFloat(amount), // Ensure float/number
        Currency: 1,
    ...
    if (res.data.status === 'error' || res.data.error) {
        // We throw the RAW error from Kesher so you can see it
...
cron.schedule('0 8 * * *', async () => {
    ...
    for (const u of users) {
        ...
        // Note: For bank we run performCharge which calls SendBankObligation (API).
        // If Kesher API charges real money via this call, great. If it just sets mandate, this logs it.
                ...
                if(canCharge) {
                    try {
                        ...
                        if (r.success) {
                            ...
                            u.donationsHistory.push({ amount: amountToCharge, note: "יומי קבוע", status: "failed", failReason: r.data?.error || JSON.stringify(r.data) || "תקלה", paymentMethod: isBank?'bank':'cc' });
                        }
        ...
        if (action === 'approve') {
            try {
                ...
                if (kesherRes.success) {
                    u.bankDetails.status = 'active'; u.bankDetails.approvedDate = new Date(); u.preferredPaymentMethod = 'bank';
                    u.messages.push({ direction: 'admin_to_user', content: 'הוראת הקבע הבנקאית אושרה.', date: new Date(), read: false });
                }
            } catch(err) {
                return res.json({ success: false, error: err.message }); // Return Exact Kesher Error
            }
        } 
...
app.post('/donate', async (req, res) => { 
    ...
    if (u.preferredPaymentMethod === 'bank') {
         if (u.bankDetails.status !== 'active') return res.json({ success: false, error: "הוראת קבע בנקאית טרם אושרה" });
         if (u.bankDetails.dailyLimit > 0 && parseFloat(amount) > u.bankDetails.dailyLimit) return res.json({ success: false, error: "חריגה מתקרה יומית" });
         
         // Use the bank creation function which hits Kesher API
         try {
             const r = await createBankObligation(u, amount, note);
             if (r.success) {
                u.totalDonated += parseFloat(amount);
                u.donationsHistory.push({ amount: parseFloat(amount), note, date: new Date(), status: 'success', isGoal: isGoalDonation === true, paymentMethod: 'bank' });
                await u.save();
                if (isGoalDonation) await GlobalGoal.findOneAndUpdate({ id: 'main_goal' }, { $inc: { currentAmount: parseFloat(amount) } });
                res.json({ success: true, message: "החיוב הבנקאי נקלט" });
             } else {
                 res.json({ success: false, error: r.data?.error || "שגיאה בבנק" });
             }
         } catch(e) {
    ...
    if (shouldChargeNow) {
        try {
            const r = await chargeCreditCard(u, amount, note, !useToken ? ccDetails : null);
            if (r.success) {
                u.totalDonated += parseFloat(amount);
                u.donationsHistory.push({ amount: parseFloat(amount), note, date: new Date(), status: 'success', isGoal: isGoalDonation === true, paymentMethod: 'cc' });
                await u.save();
                if (isGoalDonation) await GlobalGoal.findOneAndUpdate({ id: 'main_goal' }, { $inc: { currentAmount: parseFloat(amount) } });
...
// Standard Routes (Same as before)
